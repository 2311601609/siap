...
class PenerimaanProsesController extends Controller {
	...
	public function monitoring(Request $request)
	{
		...
		$sTable = "select  a.id,
					...
					left outer join t_level g on(c.kdlevel=g.kdlevel)
					left outer join t_trans h on(a.kdtran=h.id)
					where b.menu=2 and a.thang='".session('tahun')."' ".$and." ".$and1."
		...
		if($rows[0]->jml==1){
			...
			if($rows1[0]->debet>0){
				...
				if($rows1[0]->debet==$rows1[0]->kredit){
					...
					return 'Jurnal transaksi tidak balance : Debet ('.number_format($rows1[0]->debet).') | Kredit ('.number_format($rows1[0]->kredit).')';
				}
