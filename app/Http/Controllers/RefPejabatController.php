...
class RefPejabatController extends Controller {
	...
	{
		$aColumns = array('id','nama','nik','nip','nmlevel');
		...
		if(isset($_GET['sSearch'])){
			...
			if((isset($sSearch))&&($sSearch!='')){
				$sWhere=" where lower(nama) like lower('".$sSearch."%') or lower(nama) like lower('%".$sSearch."%') or
								...
								lower(nip) like lower('".$sSearch."%') or lower(nip) like lower('%".$sSearch."%') or
								lower(nmlevel) like lower('".$sSearch."%') or lower(nmlevel) like lower('%".$sSearch."%')";
			}
		}
	...
	public function simpan(Request $request)
	{
		if($request->input('inp-rekambaru')=='1'){
			...
			$insert = DB::table('t_pejabat')->insert([
