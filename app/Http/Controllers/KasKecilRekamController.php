...
class KasKecilRekamController extends Controller {
	...
	public function data(Request $request)
	{
		$sql = "
			...
			where b.menu=6 and a.thang='".session('tahun')."'
			order by a.id desc
		";
						...
						$aksi='<center>
									...
									<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 38px, 0px); top: 0px; left: 0px; will-change: transform;">
										...
										<a id="'.$row->id.'" class="dropdown-item"
											href="#/kas-kecil/lampiran?
											id_trans='.$row->id.'&
											nourut='.$row->nourut.'&
											back=kas-kecil/rekam">
											Lampiran
										</a>
									</div>
								</center>';
		...
		$rows = DB::select("
			...
			left join(
				select	id_trans,
						kdakun
				from d_trans_akun
				where grup=1 and kddk='D'
			) b on(a.id=b.id_trans)
			left join(
				select	id_trans,
						kdakun
				from d_trans_akun
				where grup=1 and kddk='K'
				...
				if($nilai>=$rows[0]->batas1 && $nilai<=$rows[0]->batas2){
					...
					if($request->input('inp-rekambaru')=='1'){
						...
						$rows = DB::select("
							...
							where a.thang=? and b.menu=6 and a.nourut=?
						",[
							session('tahun'),
							$nourut
						]);
